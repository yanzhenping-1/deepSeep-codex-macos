...
# Verify an unmanaged colliding profile is backed up instead of silently lost.
printf '# user-owned profile\n' > "$CODEX_HOME/deepseek-flash.config.toml"
...
compgen -G "$CODEX_HOME/deepseek-flash.config.toml.before-deepseek-codex-macos-*" >/dev/null
...
# Fail closed rather than silently falling back when a profile disappears.
mv "$CODEX_HOME/deepseek-flash.config.toml" "$CODEX_HOME/deepseek-flash.config.toml.hidden"
set +e
codex-deepseek-flash hello >/dev/null 2>&1
WRAPPER_STATUS=$?
set -e
[ "$WRAPPER_STATUS" -eq 78 ] || { echo "missing-profile wrapper did not fail closed" >&2; exit 1; }
mv "$CODEX_HOME/deepseek-flash.config.toml.hidden" "$CODEX_HOME/deepseek-flash.config.toml"
